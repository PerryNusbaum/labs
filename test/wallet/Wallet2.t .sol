...
contract Wallet2Test is Test {
    ...
    function testFuzz_notAllowedWithdraw(address randomUser) public {
    ...
    function testAllowedWithdraw() public {
        ...
        uint256 index = uint256(keccak256(abi.encodePacked(block.timestamp, block.prevrandao))) % 4;
    ...
    function testAllowedChangeUser() public {
        ...
        uint256 index = uint256(keccak256(abi.encodePacked(block.timestamp, block.prevrandao))) % 3;
        vm.startPrank(owner);
    ...
    function testFuzz_notAlloweChangeUser(address randomUserOther, address randomUserN) public {
        require(randomUserOther != w.getOwner());
        address[] memory old = new address[](3);
        old[0] = payable(w.getGabai1());
        old[1] = payable(w.getGabai2());
        old[2] = payable(w.getGabai3());
        uint256 index = uint256(keccak256(abi.encodePacked(block.timestamp, block.prevrandao))) % 3;
        vm.startPrank(randomUserOther);
        vm.expectRevert();
        w.changeUser(old[index], randomUserN);
        assert(w.getGabai1() == old[0] && w.getGabai2() == old[1] && w.getGabai3() == old[2]);
        vm.stopPrank();
    }
}
